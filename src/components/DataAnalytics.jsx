...
export default function DataAnalytics() {
  ...
  const tasks = [
    ...
    {
      id: "FinalsPracTask1",
      title: "Finals Practice 1",
      category: "finals",
      date: "May 15, 2026",
      description: "Hands-on data cleaning, schemas preparation, and normalization pipelines designed to establish a solid structure before importing datasets into analytical tools.",
      pdf: "/dataAnalyticsFiles/Finals PRACTICE Task 01.pdf"
    },
    {
      id: "FinalsTask1",
      title: "Finals Task 1: SQL",
      category: "finals",
      date: "May 18, 2026",
    ...
    {
      id: "FinalsEssayTask1",
      title: "Finals Essay Task 1",
      category: "finals",
      date: "May 20, 2026",
      description: "Comprehensive analytical essay discussing database normalization, robust data governance frameworks, security/privacy, and analytical storytelling methodologies.",
      pdf: "/dataAnalyticsFiles/Finals Essay Task 1.pdf"
    },
    {
      id: "FinalsTask2",
      title: "Finals Task 2: Power BI",
      category: "finals",
      date: "May 22, 2026",
      description: "Built clean, star-schema data models in Power BI desktop. Configured active relationships, normalized attributes, and designed clean structures for enterprise-level business intelligence.",
      pdf: "/dataAnalyticsFiles/Finals Task 2. Navigating Power BI.pdf"
    },
    {
      id: "FinalsTask3",
      title: "Finals Task 3: DAX",
      category: "finals",
      date: "May 23, 2026",
    ...
    {
      id: "FinalsTask4",
      title: "Finals Task 4: Interactive",
      category: "finals",
      date: "May 24, 2026",
  ...
  const handleViewLink = (url) => {
    if (url.startsWith('/')) {
      ...
      link.href = `${import.meta.env.BASE_URL}${cleanUrl}`
    ...
    <section className="py-[40px]">
      <div className="w-full px-[30px] sm:px-[60px] md:px-[120px] lg:px-[200px] xl:px-[250px] 2xl:px-[300px] mx-auto">
        <div className="flex flex-col items-center">
          <h1 className="text-[50px] font-bold text-center pb-[30px] z-2 relative" style={{ textShadow: '5px 5px 5px rgb(158, 0, 0)' }} data-aos="fade-down" data-aos-easing="linear" data-aos-duration="1500">
            Data Analytics
          </h1>
      ...
      {/* Glassmorphic Selector Tabs */}
      <div className="flex flex-row justify-center items-center gap-4 mb-[40px] px-4" data-aos="zoom-in" data-aos-duration="1200">
        {['all', 'midterm', 'finals'].map((tab) => {
          const label = tab === 'all' ? 'All' : tab === 'midterm' ? 'Midterms' : 'Finals'
          return (
            <button
              key={tab}
              onClick={() => setFilter(tab)}
              className={`px-6 py-2 rounded-[10px] font-semibold text-sm transition-all duration-300 transform active:scale-95 cursor-pointer select-none ${
                filter === tab
                  ? 'bg-red-600 text-white shadow-[0_0_15px_rgba(220,38,38,0.5)] border border-red-500'
                  : 'bg-[#222]/80 text-gray-400 hover:text-white hover:bg-[#333]/80 border border-transparent backdrop-blur-md'
              }`}
            >
              {label}
            </button>
      ...
      {/* Tasks Grid */}
      <div key={filter} className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 px-[30px] md:px-[120px] xl:px-[200px]">
        {filteredTasks.map((task) => (
          <div key={task.id} className="" data-aos="fade-up" data-aos-easing="linear" data-aos-duration="1000">
              ...
              <div className="bg-[#222] rounded-2xl p-6 shadow-xl button-85 h-full">
                <div className="flex flex-col gap-3 h-full min-h-[300px]">
                  <div className="flex flex-row justify-between items-start gap-2">
                    <h2 className="text-2xl md:text-3xl font-bold text-white text-shadow leading-tight" style={{ textShadow: '2px 2px 4px #000000' }}>
                      {task.title}
                    </h2>
                    <span className="bg-[#333] text-yellow-400 px-[12px] py-[6px] rounded-full text-[10px] font-semibold text-center whitespace-nowrap">Finished</span>
                  </div>
                  ...
                  <div className="mt-auto flex flex-row flex-wrap gap-2 justify-end items-center w-full">
                    {task.pdf && (
                      <span 
                        className="h-[35px] flex flex-row bg-[#333] text-red-400 px-3 py-2 rounded-[10px] text-[10px] sm:text-xs font-semibold cursor-pointer text-center whitespace-nowrap items-center gap-[6px] hover:bg-[#444] transition-colors" 
                        onClick={() => handleViewPDF(task.pdf)}
                      >
                        View PDF
                        <svg height="14px" width="14px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 309.267 309.267" fill="#E2574C">
                          <g>
                    ...
                    {task.excel && (
                      <span 
                        className="h-[35px] flex flex-row gap-[6px] items-center bg-[#333] text-green-400 px-3 py-2 rounded-[10px] text-[10px] sm:text-xs font-semibold cursor-pointer text-center whitespace-nowrap hover:bg-[#444] transition-colors" 
                        onClick={() => handleViewLink(task.excel)}
                      >
                        View Excel
                        <svg width="18px" height="18px" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" fill="#107C41">
                          <g>
                    ...
                    {task.sql && (
                      <span 
                        className="h-[35px] flex flex-row gap-[6px] items-center bg-[#333] text-indigo-400 px-3 py-2 rounded-[10px] text-[10px] sm:text-xs font-semibold cursor-pointer text-center whitespace-nowrap hover:bg-[#444] transition-colors" 
                        onClick={() => handleViewLink(task.sql)}
                      >
                        View SQL
                        <svg width="16px" height="16px" viewBox="0 0 24 24" fill="currentColor">
                          <path d="M12 2C6.48 2 2 4.02 2 6.5s4.48 4.5 10 4.5 10-2.02 10-4.5S17.52 2 12 2zm0 18c-5.52 0-10-2.02-10-4.5v-3.32c1.78 1.45 5.51 2.32 10 2.32s8.22-.87 10-2.32v3.32c0 2.48-4.48 4.5-10 4.5zm0-6c-5.52 0-10-2.02-10-4.5V10.7c1.78 1.45 5.51 2.32 10 2.32s8.22-.87 10-2.32v3.12c0 2.48-4.48 4.5-10 4.5z"/>
                        </svg>
                      </span>
                    )}
                    {task.powerbi && (
                      <span 
                        className="h-[35px] flex flex-row gap-[6px] items-center bg-[#333] text-yellow-400 px-3 py-2 rounded-[10px] text-[10px] sm:text-xs font-semibold cursor-pointer text-center whitespace-nowrap hover:bg-[#444] transition-colors" 
                        onClick={() => handleViewLink(task.powerbi)}
                      >
                        View Report
